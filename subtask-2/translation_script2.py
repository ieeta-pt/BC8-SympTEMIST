...
data_gs["text_translated"] = data_gs["text"].progress_apply(lambda x: tokenizer.batch_decode(model.generate(**tokenizer(x, return_tensors="pt", padding=True).to("cuda")), skip_special_tokens=True)[0])


data_gs.to_csv("dataset/medprocner_train/tsv/medprocner_tsv_train_subtask2_translated.tsv",sep="\t",index=False)